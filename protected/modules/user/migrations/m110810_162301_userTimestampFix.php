	...
	public function safeUp()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                    ...
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET create_at = FROM_UNIXTIME(createtime), lastvisit = IF(lastvisit,FROM_UNIXTIME(lastvisit),'0000-00-00 00:00:00')");
            ...
            default:
                    ...
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET create_at = datetime(createtime, 'unixepoch'), lastvisit = datetime(lastvisit, 'unixepoch')");
                    ...
                    $this->execute('INSERT INTO "'.Yii::app()->getModule('user')->tableUsers.'" SELECT "id","username","password","email","activkey","superuser","status","create_at","lastvisit" FROM "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
	...
	public function safeDown()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = UNIX_TIMESTAMP(create_at), lastvisit = UNIX_TIMESTAMP(lastvisit)");
            ...
            default:
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = strftime('%s',create_at), lastvisit = strftime('%s',lastvisit)");
                $this->execute('ALTER TABLE "'.Yii::app()->getModule('user')->tableUsers.'" RENAME TO "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
                $this->createTable(Yii::app()->getModule('user')->tableUsers, array(
                    "id" => "pk",
                    "username" => "varchar(20) NOT NULL",
